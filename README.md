...
eiscli validate -i tests/fixtures/bayesian_eis.z60 --kk-residual-threshold 3 --kk-min-points 12 --out-root result/kk
eiscli drt -i tests/fixtures/bayesian_eis.z60 --lambda 1e-3 --tau-min 1e-6 --tau-max 1e3 --n-tau 100
eiscli drt -i tests/fixtures/bayesian_eis.z60 --kk-residual-threshold 3 --kk-min-points 12
...
eiscli fit-ecm -i tests/fixtures/bayesian_eis.z60 --model R_QR --auto-init --kk-residual-threshold 3 --kk-min-points 12
...
full DRT run. Pass `--out-root DIR` to write `<stem>_kk_residuals.csv`,
`<stem>_kk_trimmed.csv`, and `<stem>_kk_summary.json`; existing files require
`--overwrite`. The residual CSV contains the raw directional residuals and the
combined pointwise relative residual percentage before trimming, plus recomputed
final residuals for retained points.

`--kk-residual-threshold PERCENT` enables conservative frequency-edge trimming.
The pointwise threshold is
`100 * hypot(residual_real_from_imag, residual_imag_from_real) / |Z_exp|`.
Only consecutive failing points at the current high- or low-frequency edge are
removed; an isolated failing interior point is retained. KK is recomputed after
each trimming round. `--kk-min-points N` (default 10, minimum 3) prevents
over-trimming. The same two options on `drt` and `fit-ecm` run this trimming step
in memory before fitting and write `kk_trim_summary.json` into the fit result
directory. The trimmed CSV written by `validate` is also a normal EIS input and
can be passed explicitly to either fitting command with `-i`.

Positive-imaginary points are removed by default; use `--keep-positive-imag` to
retain them. Interactive terminals show a compact dashboard with a score bar,
per-file details, and a batch summary. The display bands are high (at least 90),
moderate (at least 75), low (at least 50), and very low (below 50); these are
readability aids rather than pass/fail criteria.
...
| `kk_trim_summary.json` | generated by `--kk-residual-threshold`; threshold, retained/removed counts, and initial/final KK scores |
